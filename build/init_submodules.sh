...
if ! EXTRA_CMAKE_FLAGS="-DENABLE_STATIC=1 -DNO_PYVERBS=1 -DENABLE_RESOLVE_NEIGH=0" MAKEFLAGS=-j$CORES ./build.sh; then
...
cd dpdk
git -c user.name="x" -c user.email="x" am ../build/patches/dpdk/*
meson build
