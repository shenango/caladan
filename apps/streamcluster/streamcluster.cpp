...
void copycenters(Points *points, Points* centers, long* centerIDs, long offset)
{
  ...
  for ( i = 0; i < points->num; i++ ) {
    if ( is_a_median[i] ) {
      // cout << centers->p[k].coord << " " << points->p[i].coord << endl;
		    ...
		    long chunksize, long centersize, char* outfile )
{
  ...
  while(1) {
    ...
    static size_t numRead;
    // fprintf(stderr,"before read %d points\n",numRead);
    numRead  = stream->read(block, dim, chunksize );
    // fprintf(stderr,"after read %d points\n",numRead);
    ...
    std::cerr << "timestamp; " << time(NULL) << ", Points per second: " <<
	    chunksize / (elapsedMicros / 1000000.0) << std::endl;
